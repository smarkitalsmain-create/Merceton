...
interface ExportLedgerResult {
  filename: string
  csv: string
}

/**
 * Generate CSV export for merchant ledger entries.
 * Shared utility used by both merchant and admin routes.
 */
export async function exportMerchantLedgerCsv(
  params: ExportLedgerParams
): Promise<ExportLedgerResult> {
  const { merchantId, from, to, type } = params
  ...
  const filename = `merchant-ledger-${merchantId}-${y}-${m}-${d}.csv`
